...
screen_ref<-
  function(refGIS=refGIS, nlcd_year=2016, phab=NULL, natGIS=NULL, cond=NULL) {
    ...
    else
    {
      ...
      #Calucate 1st percentile and 99th percentile using quantregForest model
    ...
    refGIS3 <-refGIS2 %>%
      ...
      summarise(Ref_GIS_Ode2016 = all(screen), #This is T/F, does it pass all GIS screens?
                Ref_GISfailed_Ode2016 = sum(!screen)) %>% #This is an integer: How many screens were failed?
      ungroup()
    ...
    #HEILI: Code on line 259 aggregatese across dates. Instead we want to screen for each date You'll never have to worry about replicates for phab data.
